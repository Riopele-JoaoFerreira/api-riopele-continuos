...
function SetTableOrdem(){

    array_artigo=["artigo1","artigo2","artigo3","artigo4","artigo5","artigo6"]//string
    array_componente_principal=[]//string(?)--------------
    array_descricao=["Poliester 100%","Algodão 100%","Linho 100%","Linho 100%","Linho 100%","Linho 100%"]//string
    array_estado=[1,0,0,0,2,1]//int16(0-standby 1-selecionado 2-trabalhar)
    array_fusos=[936,850,678,940,756,809]//int16
    array_ID=[1,2,3,4,5,6]//int32(ID da Ordem)
    array_lote=["200002","200002","200003","200004","200005","200006"]//string (nº de lote)
    array_lote_componente_principal=[]//string(?)--------------
    array_ne_final=[30/1,30/1,27/1,40/1,50/1,60/1]//real32
    array_ne_real=[30/1,30/1,27/1,40/1,50/1,60/1]//real32
    array_ordem=["200005","200006","200007","200008","200009","200010"]//string (nº de ordem)
    array_quantidade_jogo=[]//real32(?)--------------
    array_quantidade_ordem=[]//real32(?)--------------
    array_sentido_torcao=["S","S","S","S","S","S"]//string
    array_tonalidade=["Azul","Vermelho","Amarelo","Verde","Amarelo","Verde"]//string
    array_torcao=[21,22,23,24,25,26]//real32
    array_velocidade_real=[]//int16(?) qual é a diferença entre os 3?--------------
    array_velocidade_media=[]//int16(?)--------------
    array_velocidade_sap=[]//int16(?)--------------

    let node_ID=''
    i_node=1
    i_array=0

    array_artigo.forEach(metodo => {
        node_ID ="NodeID;ns=4;s=ContinuosOlifil.113-101.Generic.Ordem" + i_node +"_artigo"
            var nodeToWrite = [
                {nodeId: node_ID,
                    attributeId: OPCUAClient.AttributeIds.Value,
                    value: {value: {
                            dataType: OPCUAClient.DataType.String,
                            value: array_artigo[i_array]}}}];
            session.write(nodeToWrite, function(err,statusCode,diagnosticInfo) {
                if (!err) {
                    console.log(" ARTIGO OK ");
                } else{
                    return erro;}});
            i_array=i_array+1
            i_node=i_node+1
    ...
    i_node=1
    i_array=0
    array_descricao.forEach(metodo => {
        node_ID ="NodeID;ns=4;s=ContinuosOlifil.113-101.Generic.Ordem" + i_node +"_descricao"
            var nodeToWrite = [
                {nodeId: node_ID,
                    attributeId: OPCUAClient.AttributeIds.Value,
                    value: {value: {
                            dataType: OPCUAClient.DataType.String,
                            value: array_descricao[i_array]}}}];
            session.write(nodeToWrite, function(err,statusCode,diagnosticInfo) {
                if (!err) {
...
//_______
    i_node=1
    i_array=0
    array_estado.forEach(metodo => {
        node_ID ="NodeID;ns=4;s=ContinuosOlifil.113-101.Generic.Ordem" + i_node +"_Estado"
            var nodeToWrite = [
                {nodeId: node_ID,
                    attributeId: OPCUAClient.AttributeIds.Value,
                    value: {value: {
                            dataType: OPCUAClient.DataType.Int16,
                            value: array_estado[i_array]}}}];
            session.write(nodeToWrite, function(err,statusCode,diagnosticInfo) {
                if (!err) {
    ...
    i_array=0
    console.log("ok");
    array_fusos.forEach(metodo => {
        node_ID ="NodeID;ns=4;s=ContinuosOlifil.113-101.Generic.Ordem" + i_node +"_fusos"
            var nodeToWrite = [
                {nodeId: node_ID,
                    attributeId: OPCUAClient.AttributeIds.Value,
                    value: {value: {
                            dataType: OPCUAClient.DataType.Int16,
                            value: array_fusos[i_array]}}}];
            session.write(nodeToWrite, function(err,statusCode,diagnosticInfo) {
                if (!err) {
                    console.log(" FUSOS OK" );
                } else{
                    return erro;}});
            i_array=i_array+1
            i_node=i_node+1
    ...
    i_node=1
    i_array=0
    array_ID.forEach(metodo => {
        node_ID ="NodeID;ns=4;s=ContinuosOlifil.113-101.Generic.Ordem" + i_node +"_ID"
            var nodeToWrite = [
                {nodeId: node_ID,
                    attributeId: OPCUAClient.AttributeIds.Value,
                    value: {value: {
                            dataType: OPCUAClient.DataType.Int32,
                            value: array_ID[i_array]}}}];
            session.write(nodeToWrite, function(err,statusCode,diagnosticInfo) {
                if (!err) {
                    console.log(" ID OK" );
                } else{
    ...
    i_node=1
    i_array=0        
    array_lote.forEach(metodo => {
        node_ID ="NodeID;ns=4;s=ContinuosOlifil.113-101.Generic.Ordem" + i_node +"_lote"
            var nodeToWrite = [
                {nodeId: node_ID,
                    attributeId: OPCUAClient.AttributeIds.Value,
                    value: {value: {
                            dataType: OPCUAClient.DataType.String,
                            value: array_lote[i_array]}}}];
            session.write(nodeToWrite, function(err,statusCode,diagnosticInfo) {
                if (!err) {
                    console.log(" LOTE OK" );
                } else{
    ...
    i_node=1
    i_array=0
    array_ne_final.forEach(metodo => {
        node_ID ="NodeID;ns=4;s=ContinuosOlifil.113-101.Generic.Ordem" + i_node +"_ne_final"
            var nodeToWrite = [
                {nodeId: node_ID,
                    attributeId: OPCUAClient.AttributeIds.Value,
                    value: {value: {
                            dataType: OPCUAClient.DataType.Float,
                            value: array_ne_final[i_array]}}}];
            session.write(nodeToWrite, function(err,statusCode,diagnosticInfo) {
                if (!err) {
                    console.log(" NE FINAL OK" );
                } else{
                    return erro;}});
            i_array=i_array+1
            i_node=i_node+1
    ...
    i_node=1
    i_array=0
    array_ne_real.forEach(metodo => {
        node_ID ="NodeID;ns=4;s=ContinuosOlifil.113-101.Generic.Ordem" + i_node +"_ne_real"
            var nodeToWrite = [
                {nodeId: node_ID,
                    attributeId: OPCUAClient.AttributeIds.Value,
                    value: {value: {
                            dataType: OPCUAClient.DataType.Float,
                            value: array_ne_real[i_array]}}}];
            session.write(nodeToWrite, function(err,statusCode,diagnosticInfo) {
                if (!err) {
                    console.log(" NE REAL OK" );
                } else{
                    return erro;}});
            i_array=i_array+1
            i_node=i_node+1
    ...
    i_node=1
    i_array=0
    array_ordem.forEach(metodo => {
        node_ID ="NodeID;ns=4;s=ContinuosOlifil.113-101.Generic.Ordem" + i_node +"_ordem"
            var nodeToWrite = [
                {nodeId: node_ID,
                    attributeId: OPCUAClient.AttributeIds.Value,
                    value: {value: {
                            dataType: OPCUAClient.DataType.String,
                            value: array_ordem[i_array]}}}];
            session.write(nodeToWrite, function(err,statusCode,diagnosticInfo) {
                if (!err) {
                    console.log(" ORDEM OK" );
                } else{
                    return erro;}});
            i_array=i_array+1
            i_node=i_node+1
    ...
    i_node=1
    i_array=0
    array_sentido_torcao.forEach(metodo => {
        node_ID ="NodeID;ns=4;s=ContinuosOlifil.113-101.Generic.Ordem" + i_node +"_sentido_torcao"
            var nodeToWrite = [
                {nodeId: node_ID,
                    attributeId: OPCUAClient.AttributeIds.Value,
                    value: {value: {
                            dataType: OPCUAClient.DataType.String,
                            value: array_sentido_torcao[i_array]}}}];
            session.write(nodeToWrite, function(err,statusCode,diagnosticInfo) {
                if (!err) {
                    console.log(" SENTIDO TORÇÃO OK" );
                } else{
                    return erro;}});
            i_array=i_array+1
            i_node=i_node+1
    ...
    i_node=1
    i_array=0
    array_tonalidade.forEach(metodo => {
        node_ID ="NodeID;ns=4;s=ContinuosOlifil.113-101.Generic.Ordem" + i_node +"_tonalidade"
            var nodeToWrite = [
                {nodeId: node_ID,
                    attributeId: OPCUAClient.AttributeIds.Value,
                    value: {value: {
                            dataType: OPCUAClient.DataType.String,
                            value: array_tonalidade[i_array]}}}];
            session.write(nodeToWrite, function(err,statusCode,diagnosticInfo) {
                if (!err) {
                    console.log(" TONALIDADE OK" );
                } else{
                    return erro;}});
            i_array=i_array+1
            i_node=i_node+1
    ...
    i_node=1
    i_array=0
    array_torcao.forEach(metodo => {
        node_ID ="NodeID;ns=4;s=ContinuosOlifil.113-101.Generic.Ordem" + i_node +"_torcao"
            var nodeToWrite = [
                {nodeId: node_ID,
                    attributeId: OPCUAClient.AttributeIds.Value,
                    value: {value: {
                            dataType: OPCUAClient.DataType.Float,
                            value: array_torcao[i_array]}}}];
            session.write(nodeToWrite, function(err,statusCode,diagnosticInfo) {
                if (!err) {
                    console.log(" TORÇÃO OK" );
                } else{
                    return erro;}});
            i_array=i_array+1
            i_node=i_node+1
...
exports.Set_TableOrdem = function (callback) {
    connect().then(() => {
        SetTableOrdem((res) => {
            disconnect(client); 
            return callback()
